...
static void set_state(drv_mem_t *drv, uint8_t state) {
    ...
    for(uint32_t i = 0; i < data->numpts; i++) {
        // printf("pd %u\n", i);
        pt_set_entry((void*)0xFFFFF000, (uint16_t)((data->addr_start >> 22) + i), get_physaddr((uint32_t)data->pts[i]), 
...
static void free_page(drv_mem_t *drv, void *addr, uint32_t size) {
    ...
    for(uint32_t n = 0; n < size; n++) {
        ...
        uint32_t phys = data->pts[pt_entry >> 10][pt_entry & 0x3FF] & 0xFFFFF000;
        if(phys < 0x800000 || phys >= ((data->total_range * 4096 * 32) + 0x800000)) continue;
        ...
        pt_set_entry(data->pts[pt_entry >> 10], (uint16_t)(pt_entry & 0x3FF), 0, 0, 0, 0);
...
static void *alloc_page(drv_mem_t *drv, void *addr, uint32_t size) {
    ...
    for(uint32_t n = 0; n < size; n++) {
        ...
        uint32_t pt_entry = (((uint32_t)addr - data->addr_start) >> 12) + n;
        pt_set_entry(data->pts[pt_entry >> 10], (uint16_t)(pt_entry & 0x3FF), (free << 12) + 0x800000, 
