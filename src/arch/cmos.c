...
uint32_t to_epoch(uint8_t s, uint8_t m, uint8_t h, uint8_t d, uint8_t mo,
                  uint8_t y) {
    const uint16_t doy[12]
      = {0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334};
    uint32_t day = d + doy[mo - 1] - 1, year = y + 2000 - 1900;
    uint32_t res = s + m * 60 + h * 3600 + day * 86400 + (year - 70) * 31536000
                   + ((year - 69) / 4) * 86400 - ((year - 1) / 100) * 86400
                   + ((year + 299) / 400) * 86400;
    return res;
}

static void rtc_read(uint8_t *seconds, uint8_t *minutes, uint8_t *hours,
...
static void print_rtc(void) {
    ...
    fattime = ((data[5] + 2000 - 1980) << 25) | (data[4] << 21)
              | (data[3] << 16) | (data[2] << 11) | (data[1] << 5)
              | (data[0] >> 1);
    printf("%u:%02u:%02u %u.%02u.%02u %lu\n", data[2], data[1], data[0],
           data[3], data[4], data[5], epoch);
}
