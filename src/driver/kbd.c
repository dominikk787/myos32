...
static uint16_t set1_scancode_to_ascii(drv_in_t *drv, uint16_t code) {
    ...
    static const unsigned char map[] = {
      ...
      0, ' ', 0, IN_KEY_F1, IN_KEY_F2, IN_KEY_F3, IN_KEY_F4,
      IN_KEY_F5,  // 38
                  // 45h to Num Lock, 46h to Scroll Lock
      IN_KEY_F6, IN_KEY_F7, IN_KEY_F8, IN_KEY_F9, IN_KEY_F10, 0, 0,
      IN_KEY_HOME,  // 40
      IN_KEY_UP, IN_KEY_PGUP, '-', IN_KEY_LFT, '5', IN_KEY_RT, '+',
      IN_KEY_END,  // 48
      IN_KEY_DN, IN_KEY_PGDN, IN_KEY_INS, IN_KEY_DEL, 0, 0, 0,
      IN_KEY_F11,   // 50
      IN_KEY_F12};  // 58
    static const unsigned char shift_map[]
      = {0,          0x1B,        '!',        '@',
         ...
         '>',        '?',         0,          0,  // 30
         0,          ' ',         0,          IN_KEY_F1,
         IN_KEY_F2,  IN_KEY_F3,   IN_KEY_F4,  IN_KEY_F5,  // 38
         IN_KEY_F6,  IN_KEY_F7,   IN_KEY_F8,  IN_KEY_F9,
         IN_KEY_F10, 0,           0,          IN_KEY_HOME,  // 40
         IN_KEY_UP,  IN_KEY_PGUP, '-',        IN_KEY_LFT,
         '5',        IN_KEY_RT,   '+',        IN_KEY_END,  // 48
         IN_KEY_DN,  IN_KEY_PGDN, IN_KEY_INS, IN_KEY_DEL,
         0,          0,           0,          IN_KEY_F11,  // 50
         IN_KEY_F12};                                      // 58
    uint16_t temp;
    ...
    LEDS:
        ...
        temp = 0;
        if(data->kbd_status & KBD_META_SCRL) temp |= 1;
        if(data->kbd_status & KBD_META_NUM) temp |= 2;
        if(data->kbd_status & KBD_META_CAPS) temp |= 4;
        write_kbd(0x60, temp);
        return 0xFFFF;
    }
    ...
    if(data->kbd_status & IN_SPECIAL_SHIFT) {
        ...
        if((data->kbd_status & KBD_META_CAPS)
           || ((data->kbd_status & IN_SPECIAL_ANY) != IN_SPECIAL_SHIFT)) {
            if(temp >= 'A' && temp <= 'Z') temp = map[code];
        }
    } else {
        if(code >= sizeof(map) / sizeof(map[0])) return 0xFFFF;
        temp = map[code];
        if(temp == 0) return 0xFFFF;
        if((data->kbd_status & KBD_META_CAPS)
           && ((data->kbd_status & IN_SPECIAL_ANY) == 0)) {
            if(temp >= 'a' && temp <= 'z') temp = shift_map[code];
        }
    }
