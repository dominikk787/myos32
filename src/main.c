...
drv_screen_graphic_data_t screen0g_data = {.font = (void*)psf_font_lat2_vga16, .cursor_start = 11, .cursor_end = 13};
...
drv_pagealloc_data_t pagealloc_data = {.pts = &pagealloc_tpts, .numpts = 1, .map = pagealloc_map, .total_range = 1024, 
                                        .addr_start = 0xDFC00000, .flags = 0};
...
drv_pagealloc_data_t malloc_data = {.pts = malloc_tpts, .numpts = 1, .map = (void*)0xDFC00000, .total_range = 0, 
...
static uint32_t multiboot_fb_width, multiboot_fb_height, multiboot_fb_bpp, multiboot_fb_pitch, multiboot_mem_low, multiboot_mem_high;
    ...
    for (tag = (struct multiboot_tag *) (mb_addr + 8); tag->type != MULTIBOOT_TAG_TYPE_END;
        ...
        case MULTIBOOT_TAG_TYPE_MMAP: {
            ...
            for (mmap = ((struct multiboot_tag_mmap *) tag)->entries; (uint8_t *) mmap  < (uint8_t *) tag + tag->size;
                ...
                printf (" base_addr = 0x%lx%lx, length = 0x%lx%lx, type = 0x%lx\n",
        ...
        case MULTIBOOT_TAG_TYPE_FRAMEBUFFER: {
            ...
            case MULTIBOOT_FRAMEBUFFER_TYPE_INDEXED: {
                ...
                for (i = 0; i < tagfb->framebuffer_palette_num_colors; i++) {
                    distance = (uint32_t)((0xff - palette[i].blue) * (0xff - palette[i].blue)) + (palette[i].red * palette[i].red) + 
            ...
            case MULTIBOOT_FRAMEBUFFER_TYPE_RGB:
                color = (uint32_t)((1 << tagfb->framebuffer_blue_mask_size) - 1) << tagfb->framebuffer_blue_field_position;
                break;
...
void parse_multiboot(void) {
    ...
    for (tag = (struct multiboot_tag *) (mb_addr + 8); tag->type != MULTIBOOT_TAG_TYPE_END;
        ...
        //     break;
        case MULTIBOOT_TAG_TYPE_BASIC_MEMINFO:
            multiboot_mem_low = ((struct multiboot_tag_basic_meminfo *) tag)->mem_lower;
            multiboot_mem_high = ((struct multiboot_tag_basic_meminfo *) tag)->mem_upper;
            break;
        // case MULTIBOOT_TAG_TYPE_BOOTDEV:
        //     printf ("Boot device 0x%x,%u,%u\n",
        //         ((struct multiboot_tag_bootdev *) tag)->biosdev,
        ...
        case MULTIBOOT_TAG_TYPE_FRAMEBUFFER: {
            struct multiboot_tag_framebuffer *tagfb = (struct multiboot_tag_framebuffer *) tag;
            multiboot_fb = (void *) (unsigned long) tagfb->common.framebuffer_addr;
    ...
    if((flags & (IN_SPECIAL_ALT | IN_SPECIAL_CTRL)) == 0 && ch >= ' ' && ch < IN_KEY_F1) {
        if(ch == 'M') test_multiboot();
            ...
            for(uint32_t i = 0; i < 80*25; i++) {
                ((uint16_t*)0xC00B8000)[i] = (uint16_t)(((i / 80) & 0x7) << 12 | (i & 1 ? 1 << 15 : (i < 256 ? i : 0)) | OUT_COLOR_WHITE << 8);
            }
        ...
        if(ch >= ' ' && ch < IN_KEY_F1) printf("%c", ch);
            ...
            if(flags == 0) {
                if(ch == IN_KEY_F1) {
                    ...
                    screen0.clear(&screen0);
                    // ext2_print_inodes(&ext2_data);
                    // ext2_dump_inode(&ext2_data, ext2_find_inode(&ext2_data, 2, "/grub/fonts/unicode.pf2"));
                    ext2_dump_inode(&ext2_data, ext2_find_inode(&ext2_data, 2, "/test"));
...
static void init_screen(void) {
    ...
    pt_set_entry((void*)0xFFFFF000, 0xD00 >> 2, get_physaddr((uint32_t)pt_vga), 0, 1, 1);
    for(uint16_t i = 0; i < pages; i++) {
        pt_set_entry(pt_vga, i, (uint32_t)multiboot_fb + (i * 4096), 0, 1, 1);
        invlpg(0xD0000000 + (i * 4096));
    }
    if(multiboot_fb_bpp == 32) {
        float scale = sqrtf((float)(multiboot_fb_width*multiboot_fb_width + multiboot_fb_height*multiboot_fb_height));
        for(uint16_t y = 0; y < multiboot_fb_height; y++) {
            uint32_t *line = (uint32_t*)(0xD0000000 + (y * multiboot_fb_pitch));
            for(uint16_t x = 0; x < multiboot_fb_width; x++) {
                float r = sqrtf((float)(x*x + y*y)) / scale * 256, g = sqrtf((float)((multiboot_fb_width - 1 - x)*(multiboot_fb_width - 1 - x) + y*y)), 
                    b = sqrtf((float)(x*x + (multiboot_fb_height - 1 - y)*(multiboot_fb_height - 1 - y)));
                uint8_t rb = (uint8_t)min((uint16_t)r, 255), gb = (uint8_t)min((uint16_t)g, 255), bb = (uint8_t)min((uint16_t)b, 255);
        ...
        for(uint16_t y = 0; y < multiboot_fb_height; y++) {
            uint16_t *line = (uint16_t*)(0xD0000000 + (y * multiboot_fb_pitch));
            for(uint16_t x = 0; x < multiboot_fb_width; x++) {
                line[x] = 0;
            }
        }
...
void start_kernel(uint32_t magic, uint32_t addr) {
    ...
    write_serial('c');
    init_screen();
    printf("fb: width %lu height %lu bpp %lu pitch %lu addr %08lX text %u\n", multiboot_fb_width, multiboot_fb_height, 
        multiboot_fb_bpp, multiboot_fb_pitch, (uint32_t)multiboot_fb, multiboot_fb_text);
    write_serial('d');
    init_pit();
    init_kbd();
    rtc_init();
    ...
    if(mbr[446 + 4] == 0x83) ext2_init(&ext2_data, ((uint32_t*)(mbr + 446))[2]);
    else {
        ...
        FIL fil;
        UINT written;
        FRESULT res = f_open(&fil, "0:/test.txt", FA_READ | FA_WRITE | FA_OPEN_APPEND);
        if(res) goto fs_err;
        f_write(&fil, "abcd\n", 5, &written);
        printf("written %u\n", written);
        f_close(&fil);
